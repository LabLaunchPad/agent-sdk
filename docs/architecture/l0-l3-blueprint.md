...
**Target users / jobs-to-be-done.** Developers building production agents who have hit two concrete failure modes: (1) *"When my agent runs long enough to crash, I want it to resume exactly where it left off, without running infrastructure"* — the gap the LangGraph "checkpointing isn't production-grade durability" criticism documents; (2) *"When my agent calls third-party MCP tools, I want injection/poisoning defenses built into the call path, not left to me"* — the gap the tool-poisoning/indirect-injection evidence below documents. This narrow wedge is why Kernel and Tool Runtime are the first two SDKs rather than a broader platform play; wider agent-platform users are a Horizon 2+ concern once those two are proven.
...
| Tool Runtime isolation/sandbox substrate (where untrusted tool code actually executes) | **Wrap an existing hardened substrate — not a bespoke Rust sandbox** | EXPERIMENTAL — narrowed shortlist, still needs a prototype before MVP freeze (ADR-F) | MEDIUM | Rust's memory safety is a real property but is **not equivalent to sandbox/process security**. Building a custom native sandbox remains a high-build-risk, low-solo-founder-maintainability bet to avoid without proven necessity. See ADR-F for the narrowed shortlist (Deno-sandboxed subprocess for Tier 1; gVisor/Firecracker for Tier 2 on Linux, with an explicit reduced-isolation fallback on macOS/Windows dev machines) and the cross-platform gap that shortlist resolves. |
...
- **ADR-F (Tool Runtime sandbox substrate, deep dive).** Options evaluated: in-process JS sandbox (vm2/isolated-vm), WASM/WASI, Deno-as-subprocess-runtime, gVisor, Firecracker, plain Docker container. Evidence: vm2 has a long, ongoing CVE history and its own maintainer now recommends against relying on it — in-process JS sandboxing is not a durable security boundary. WASM/WASI gives a memory-safe, capability-gated sandbox, production-ready for stateless compute-focused executors, but fits first-party tool code compiled to WASM better than arbitrary third-party MCP server processes. Deno has shipped a default-deny, capability-based permission model since 2020; Node's equivalent only reached stable in Node 23.5 (Dec 2024) and is less battle-tested — so Tier 1 should wrap Deno as the tool-execution subprocess runtime rather than hand-roll `child_process` + OS-permission logic, while Node stays the orchestration runtime per ADR-A. Industry has converged on microVMs over plain containers for genuinely untrusted code: Firecracker gives hardware-level isolation via KVM (~125ms boot, <5MiB overhead) at the strongest guarantee; gVisor gives syscall-level interception, faster startup, weaker isolation, 10-30% I/O overhead. **Counter-evidence / local-first gap:** both gVisor and Firecracker require Linux + KVM and do not run natively on macOS or Windows — exactly the platforms a local-first SDK needs to support on developer laptops; this was not surfaced in the original Tier 2 sketch. *Decision (narrows the shortlist, does not fully lock it):* Tier 1 = Deno-sandboxed subprocess (default-deny); Tier 2 on Linux/cloud targets = gVisor by default, Firecracker for the highest-risk hosted multi-tenant case later; Tier 2 on macOS/Windows dev machines = plain container + Deno permissions, with the reduced isolation explicitly documented rather than assumed equivalent to the Linux path. Confidence: MEDIUM — narrows the prototype scope significantly but, per the Prototype-First Rule, is not LOCKED without an actual escape-resistance/latency/operability benchmark. Revisit when: the prototype runs, or if Node's native permission model matures enough that wrapping a second runtime (Deno) stops paying for itself.
...
| Language/runtime — Tool Runtime sandbox substrate | **NOT FROZEN** — EXPERIMENTAL; shortlist narrowed by ADR-F (Deno-sandboxed subprocess for Tier 1; gVisor/Firecracker for Tier 2 on Linux with a documented reduced-isolation fallback on macOS/Windows), still blocked on the actual prototype (see Verification section) |
...
- Sandbox prototype (per ADR-F): benchmark (a) Deno-sandboxed subprocess for Tier 1, (b) gVisor for Tier 2 on Linux, and (c) the macOS/Windows plain-container-+-Deno-permissions fallback, on startup latency, escape resistance (attempted FS/network access from a malicious tool), and solo-founder operability (setup/debugging burden across all three host platforms) — decision recorded as an update to ADR-F once measured.
