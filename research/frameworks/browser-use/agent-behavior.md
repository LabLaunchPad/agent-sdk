...
title: Browser Use — Agent Behavior
description: 'agent-behavior dimension research for browser-use, imported from a prior external research pass'
sources:
...
The documented SDK loop is intentionally thin; `client.run()` creates a session and polls for completion, while `Agent.run()` drives the task. Cloud sessions can last for long periods. Skills encode repeated behavior into reusable calls.

**Unknown:** exact internal stopping/replanning policy is not fully exposed by the reviewed public docs, so do not infer more than the observable API guarantees.
