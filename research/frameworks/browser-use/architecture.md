...
description: 'architecture dimension research for browser-use, imported from a prior external research pass'
sources:
...
The core lesson is **capability density in a narrow loop**: task -> model -> browser action -> observation -> repeat. BrowserCode further demonstrates a minimal primitive where the agent writes JavaScript that executes against a persistent browser session.

**Counterexample:** many agent-framework abstractions can be omitted when the environment itself supplies rich capabilities.
