...
title: Browser Use — State
description: 'state dimension research for browser-use, imported from a prior external research pass'
sources:
...
State is centered on browser/session/workspace lifecycle rather than a general durable agent identity. Cloud API responses and skill execution support session IDs and reusable browser profiles.

LabLaunchPad should borrow the principle of narrow state surfaces per capability domain.
