...
title: CrewAI — Agent Behavior
description: 'agent-behavior dimension research for crewai, imported from a prior external research pass'
sources:
...
Agents can delegate within crews; tasks can depend on other tasks and run asynchronously; crews support sequential or hierarchical execution. Flows provide conditional/event-driven state transitions.

**Coordination cost hypothesis:** additional role/manager/model calls must earn their cost via better verified outcomes. This should be benchmarked rather than assumed.
