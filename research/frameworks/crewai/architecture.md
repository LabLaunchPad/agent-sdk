...
description: 'architecture dimension research for crewai, imported from a prior external research pass'
sources:
...
Core stack: **Agent(role/goal/backstory/tools) -> Task(expected output/context) -> Crew(process) / Flow(stateful routing) -> Memory/Knowledge/Guardrails -> Runtime/Observability**.

Sequential processes are linear; hierarchical processes add a manager agent that delegates and validates. Flows add explicit event-driven state.

**Tradeoff:** role abstractions help social coordination but may be redundant for single-agent tasks.
