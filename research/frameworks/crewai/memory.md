...
title: CrewAI — Memory
description: 'memory dimension research for crewai, imported from a prior external research pass'
sources:
...
CrewAI has a built-in memory subsystem and knowledge tooling, with configurable embedders. The presence of both memory and workflow state reinforces the need to keep durable state separate from semantic memories.

**Risk:** embedding/retrieval defaults can create hidden cost and stale-context behavior unless observable.
