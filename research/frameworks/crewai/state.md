...
title: CrewAI — State
description: 'state dimension research for crewai, imported from a prior external research pass'
sources:
...
Flows explicitly model typed state and support persistence/resume in the current docs. Crew execution also exposes usage metrics.

LabLaunchPad should use typed state only where deterministic control benefits; avoid making every Agent carry workflow state.
