...
title: Google Agent Development Kit (ADK) — Agent Behavior
description: 'agent-behavior dimension research for google-adk, imported from a prior external research pass'
sources:
...
# Google Agent Development Kit (ADK) — Agent Behavior
...
ADK supports single agents, managed agents, multi-agent teams and explicit workflow patterns including sequential, loop, parallel and routing workflows. The event loop and runtime support resume/cancel semantics.

**Strength:** deterministic structure around probabilistic steps.
**Risk:** graph/workflow abstractions can grow faster than the core use case; measure concept load.
