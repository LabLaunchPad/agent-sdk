...
title: Google Agent Development Kit (ADK) — Architecture
description: 'architecture dimension research for google-adk, imported from a prior external research pass'
sources:
...
ADK spans **Agent -> Tool/Callbacks -> Session/Event/State -> Context services -> Runtime -> Deployment/Observability/Evaluation**. The current documentation also introduces graph-based workflows that mix deterministic code with adaptive reasoning.

This is a useful reference for LabLaunchPad's separation of orchestration from runtime. Avoid importing every Google platform concept into the core contract.
