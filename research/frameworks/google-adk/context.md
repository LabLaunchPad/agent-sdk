...
title: Google Agent Development Kit (ADK) — Context
description: 'context dimension research for google-adk, imported from a prior external research pass'
sources:
...
# Google Agent Development Kit (ADK) — Context
...
ADK states that context is treated like source code: sessions, memory, tool outputs and artifacts are assembled; irrelevant events may be filtered, old turns summarized, artifacts lazy-loaded, and token usage tracked. This directly supports a context broker/progressive disclosure strategy.

**Benchmark lesson:** context efficiency should be measured by outcome per token, not raw context size.
