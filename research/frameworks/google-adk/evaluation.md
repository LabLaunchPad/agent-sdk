...
title: Google Agent Development Kit (ADK) — Evaluation
description: 'evaluation dimension research for google-adk, imported from a prior external research pass'
sources:
...
# Google Agent Development Kit (ADK) — Evaluation
...
ADK has unusually explicit evaluation docs. Criteria include tool-trajectory matching and response matching, with configurable thresholds; evaluation also supports simulation/custom metrics/optimization in the current docs.

**Adopt:** trajectory correctness as a separate signal from final text quality.
**Need reproduction:** verify exact scoring behavior against current implementation before freezing LabLaunchPad contracts.
