...
description: 'overview dimension research for google-adk, imported from a prior external research pass'
sources:
...
Google ADK is a broad production-oriented framework: multi-language SDKs, graph workflows, sessions/state/events, context management, evaluation, observability, deployment and A2A integration. Current docs explicitly position it as optimized for Gemini but model- and deployment-agnostic, with adapters for other providers and local models.

**Primary JTBD:** build, test, evaluate and deploy production agents with a structured runtime.
**Unique strength:** lifecycle completeness and deterministic workflow composition alongside model reasoning.
