...
title: Google Agent Development Kit (ADK) — Runtime
description: 'runtime dimension research for google-adk, imported from a prior external research pass'
sources:
...
# Google Agent Development Kit (ADK) — Runtime
...
The documented deployment range includes local/containerized operation and Google Agent Runtime, Cloud Run and GKE. The runtime supports web interface/API server/event loop, resume/cancel, and observability.

**Neutral extraction:** Runtime should be a replaceable execution service, not embedded inside Agent semantics.
