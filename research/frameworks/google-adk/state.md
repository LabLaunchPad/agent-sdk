...
title: Google Agent Development Kit (ADK) — State
description: 'state dimension research for google-adk, imported from a prior external research pass'
sources:
...
# Google Agent Development Kit (ADK) — State
...
Sessions are the durable conversation/runtime boundary and can contain state plus events; session services abstract storage. Current Java reference documents session creation, event appending and state deltas.

**LabLaunchPad lesson:** a Session should be a storage-independent contract with an event log and state delta semantics.
