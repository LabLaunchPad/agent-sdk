...
title: Google Agent Development Kit (ADK) — Ux
description: 'ux dimension research for google-adk, imported from a prior external research pass'
sources:
...
# Google Agent Development Kit (ADK) — Ux
...
ADK's observable runtime concepts—runs, events, traces, evaluation, deployment and cancellation—support an operator mental model based on lifecycle state rather than chat only.

Adopt explicit RunStatus transitions and event timeline views.
