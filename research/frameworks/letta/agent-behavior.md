...
title: Letta Agent SDK / Letta — Agent Behavior
description: 'agent-behavior dimension research for letta, imported from a prior external research pass'
sources:
...
# Letta Agent SDK / Letta — Agent Behavior
...
The key behavioral distinction is that the agent can modify its own memory via built-in memory tools. Runtime behavior therefore includes a write-back path from reasoning to durable context.

**Strength:** continuity and personalization.
**Risk:** memory poisoning/staleness becomes a first-order failure mode. A durable memory write should require provenance, confidence, scoping and possibly policy review.
