...
title: Letta Agent SDK / Letta — Architecture
description: 'architecture dimension research for letta, imported from a prior external research pass'
sources:
...
# Letta Agent SDK / Letta — Architecture
...
Core conceptual stack: **AgentState + Memory Blocks + Session/Conversation + Tools + Backend**. AgentState is persisted and documented as sufficient to recreate a persisted agent. Memory blocks are attached to agents and can be shared/detached dynamically.

The architecture makes durable state semantically visible, which is valuable for LabLaunchPad even if the implementation differs.
