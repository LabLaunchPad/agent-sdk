...
title: Letta Agent SDK / Letta — Ax
description: 'ax dimension research for letta, imported from a prior external research pass'
sources:
...
AI coding-agent usability improves when the agent model itself is explicit and machine-discoverable. The current SDK README exposes browser/React Native portable entry points and management APIs, which suggests good platform awareness.

LabLaunchPad should likewise make its durable-state contracts visible to coding agents through exact schemas and examples.
