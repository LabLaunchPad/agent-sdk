...
title: Letta Agent SDK / Letta — Capabilities
description: 'capabilities dimension research for letta, imported from a prior external research pass'
sources:
...
# Letta Agent SDK / Letta — Capabilities
...
Capabilities include custom tools, MCP/out-of-process tools, files, archival memory and shared blocks. Docker deployments can optionally sandbox custom tools via E2B; MCP tools execute outside the Letta server.

Classification: **LOCAL + REMOTE + HYBRID**. The critical security boundary is not only the agent server but the execution environment of tools and MCP servers.
