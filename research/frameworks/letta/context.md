...
title: Letta Agent SDK / Letta — Context
description: 'context dimension research for letta, imported from a prior external research pass'
sources:
...
Letta's context hierarchy is a progressive-disclosure model: always-visible blocks for critical state, file retrieval for larger structured sources, archival retrieval for less salient memories, and external RAG for scale.

**Caution:** “always visible” is efficient for small critical state but can become token tax and stale-context risk at scale. Treat visibility as a policy, not a universal default.
