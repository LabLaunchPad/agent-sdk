...
title: Letta Agent SDK / Letta — Dx
description: 'dx dimension research for letta, imported from a prior external research pass'
sources:
...
DX strength is conceptual clarity for stateful agents: Agent, blocks, sessions, client backends. The tradeoff is that memory semantics add an operational burden: block schemas, limits, write policies, lifecycle and provenance.

**Progressive complexity rule:** a first-run agent should work without custom memory engineering.
