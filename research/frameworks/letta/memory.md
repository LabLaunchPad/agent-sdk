...
title: Letta Agent SDK / Letta — Memory
description: 'memory dimension research for letta, imported from a prior external research pass'
sources:
...
Memory blocks are structured sections of the context window, always visible while attached, with optional read-only mode; they can be shared between agents. Letta's context hierarchy recommends memory blocks for small/high-importance state, files for larger read-only data, archival memory for lower-recall items, and external RAG for very large corpora.

This is a strong evidence-backed pattern for separating **memory vs knowledge vs context** instead of treating them as synonyms.
