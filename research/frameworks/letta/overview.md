...
title: Letta Agent SDK / Letta — Overview
description: 'overview dimension research for letta, imported from a prior external research pass'
sources:
...
# Letta Agent SDK / Letta — Overview
...
Letta is the clearest stateful-agent counterexample: durable agent identity and memory are part of the core model rather than an add-on. Its current Agent SDK provides one TypeScript interface across managed/local/self-hosted deployments, and the platform explicitly differentiates in-context memory blocks, files, archival memory and external RAG.

**Primary JTBD:** build agents that persist state and memory across interactions and can learn/change over time.
**Best fit:** personal assistants, coding agents, long-lived coworkers.
**Unique strength:** explicit memory lifecycle and agent state persistence.
