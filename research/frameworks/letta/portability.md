...
title: Letta Agent SDK / Letta — Portability
description: 'portability dimension research for letta, imported from a prior external research pass'
sources:
...
# Letta Agent SDK / Letta — Portability
...
Mapping: Letta AgentState -> LabLaunchPad DurableAgentState; memory blocks -> scoped Memory objects; session -> Run/Conversation; archival/RAG -> Knowledge/Evidence providers.

Do not merge AgentState with Memory. Memory is a semantic content layer; state also contains configuration and identity required to recreate the agent.
