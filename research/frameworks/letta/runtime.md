...
title: Letta Agent SDK / Letta — Runtime
description: 'runtime dimension research for letta, imported from a prior external research pass'
sources:
...
Letta can run locally via Docker with persistent PostgreSQL-backed data and can connect to several model providers; managed cloud and remote/server configurations are also documented.

**Neutral runtime lesson:** durability requires an explicit storage backend contract and restart semantics; a “stateless API wrapper” is insufficient for long-lived agents.
