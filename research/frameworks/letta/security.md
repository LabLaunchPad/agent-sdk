...
title: Letta Agent SDK / Letta — Security
description: 'security dimension research for letta, imported from a prior external research pass'
sources:
...
# Letta Agent SDK / Letta — Security
...
Read-only memory blocks provide one clean policy primitive: the agent can consume organizational context without writing to it. Dynamic attach/detach allows access to be revoked. Tool sandboxing is available, but MCP tools are outside the Letta server boundary.

**Requirement:** classify memory writes, tool execution and remote MCP as separate trust zones.
