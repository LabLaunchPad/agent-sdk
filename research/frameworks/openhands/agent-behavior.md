...
title: OpenHands Software Agent SDK — Agent Behavior
description: 'agent-behavior dimension research for openhands, imported from a prior external research pass'
sources:
...
# OpenHands Software Agent SDK — Agent Behavior
...
Observed loop: user input -> context/event history -> LLM action generation -> action security/confirmation -> tool execution -> observation event -> next decision. OpenHands also documents goal-completion and stuck-detection mechanisms, plus forkable conversations and task delegation.

**Anti-drift primitives to borrow:** explicit completion judge, stuck detector, event log, resumability, pending-action confirmation.
**Failure modes:** model-generated false security classification; direct tool execution paths can bypass the conversation-level analyzer; unknown security risk needs policy handling. These are documented limitations, not proof of universal exploitability.
