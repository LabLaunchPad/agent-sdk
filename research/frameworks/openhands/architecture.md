...
title: OpenHands Software Agent SDK — Architecture
description: 'architecture dimension research for openhands, imported from a prior external research pass'
sources:
...
# OpenHands Software Agent SDK — Architecture
...
The documented decomposition is approximately **Agent -> Conversation -> Event log -> Tool/Workspace -> LLM/Context/Skills**, with Agent Server providing a remote execution boundary. The Agent is described as a stateless reasoning-action loop while Conversation owns persistent event history.

The high-value architectural pattern is event separation: action/observation events make the loop inspectable and allow runtime-level versus tool-level failures to be distinguished.

**Essential:** agent loop, event/state boundary, tool boundary, workspace, persistence. **Convenience/platform layers:** CLI, cloud workspace, particular UI integrations. **Potentially unstable abstraction:** model-specific prompting/skill syntax.
