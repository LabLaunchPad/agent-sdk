...
title: OpenHands Software Agent SDK — Capabilities
description: 'capabilities dimension research for openhands, imported from a prior external research pass'
sources:
...
# OpenHands Software Agent SDK — Capabilities
...
Strong built-in software capabilities include shell/bash, file operations, browser-enabled workspaces, MCP, custom tools, skills, task/sub-agent tooling and remote Agent Server execution. Local and ephemeral workspace modes are explicitly supported.

Classification: **LOCAL_NATIVE + REMOTE + HYBRID**. Danger boundaries include shell, filesystem writes/deletes, networked commands, secret registry and MCP.

**Requirement:** every capability should expose permission scope, resource policy, provenance, approval policy and audit hooks.
