...
title: OpenHands Software Agent SDK — Context
description: 'context dimension research for openhands, imported from a prior external research pass'
sources:
...
# OpenHands Software Agent SDK — Context
...
Skills can activate on every step or based on keywords/task triggers, and can inject dynamic content and MCP configuration. Conversation events form the primary context history; docs also expose condensation/compression elsewhere in the SDK ecosystem.

**LabLaunchPad lesson:** progressive disclosure plus explicit activation is preferable to loading every instruction/tool by default. Pair with a repo retrieval/context broker.
