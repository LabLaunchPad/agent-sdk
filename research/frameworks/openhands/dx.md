...
title: OpenHands Software Agent SDK — Dx
description: 'dx dimension research for openhands, imported from a prior external research pass'
sources:
...
# OpenHands Software Agent SDK — Dx
...
DX strengths: composable Python SDK, examples, typed event architecture, REST/remote Agent Server, explicit workspace types, skills and security APIs. DX cost: many concepts (Agent, Conversation, events, workspace, security analyzer, confirmation policy, skills, MCP, persistence).

**Migration benchmark:** measure time-to-first-agent, time-to-first-tool, and time-to-first-resumable-run with a deliberately minimal and a fully governed setup.
