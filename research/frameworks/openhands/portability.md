...
title: OpenHands Software Agent SDK — Portability
description: 'portability dimension research for openhands, imported from a prior external research pass'
sources:
...
# OpenHands Software Agent SDK — Portability
...
Semantic mapping: OpenHands Agent -> LabLaunchPad Agent + Harness; Conversation -> Run + EventLog; Workspace -> WorkspaceContract; Agent Server -> Runtime; Skills -> Capability/Context package; Security Analyzer -> Policy Engine.

No drop-in compatibility is recommended. Preserve behavioral semantics, not class names.
