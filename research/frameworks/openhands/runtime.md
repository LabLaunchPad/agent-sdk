...
title: OpenHands Software Agent SDK — Runtime
description: 'runtime dimension research for openhands, imported from a prior external research pass'
sources:
...
# OpenHands Software Agent SDK — Runtime
...
The Agent Server is a remote runtime boundary; DockerWorkspace can isolate an agent server inside a container and clean up lifecycle automatically. OpenHands Cloud Workspace supplies managed sandboxes. The local machine can also be used directly as the workspace.

**Neutral extraction:** WorkspaceContract should define root, mount mode, durability, artifacts, cleanup, network and credential injection. SandboxContract should define isolation strength, resource limits, network policy and escape reporting.
