...
title: OpenHands Software Agent SDK — Security
description: 'security dimension research for openhands, imported from a prior external research pass'
sources:
...
# OpenHands Software Agent SDK — Security
...
Security uses risk levels plus a confirmation policy. The docs expose deterministic analyzers (pattern/policy rails) and optional LLM analysis, with an ensemble selecting the highest concrete risk. The documentation explicitly says these are not a complete prompt-injection solution or sandbox replacement.

**Strong pattern:** analyze at the action boundary before execution.
**Critical limitation:** `execute_tool()` bypasses conversation analyzers/confirmation, showing why capability enforcement must live below the agent loop for high-assurance environments.
