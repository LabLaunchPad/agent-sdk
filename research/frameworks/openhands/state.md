...
title: OpenHands Software Agent SDK — State
description: 'state dimension research for openhands, imported from a prior external research pass'
sources:
...
# OpenHands Software Agent SDK — State
...
Conversation state is durable and can include message history, agent configuration, execution state, tool outputs, usage statistics, workspace context, activated skills, secrets, and agent state. This is a strong example of **runtime state as a first-class persisted object**.

For LabLaunchPad, do not collapse this into Memory. Model at least: RunState, EventLog, WorkspaceState, CapabilityState, and Memory separately.
