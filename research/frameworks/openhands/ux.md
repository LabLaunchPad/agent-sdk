...
title: OpenHands Software Agent SDK — Ux
description: 'ux dimension research for openhands, imported from a prior external research pass'
sources:
...
# OpenHands Software Agent SDK — Ux
...
The UX pattern is a visible run/conversation with events, approval waits, errors, resumability and workspace operations. The best reusable interaction pattern is **“agent is working” as an inspectable state machine**, not a chat transcript only.

Do not copy visuals; copy state visibility: pending approval, current tool/action, last observation, resource usage, recovery/retry state.
