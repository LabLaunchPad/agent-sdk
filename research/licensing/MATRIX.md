...
| **Mastra**                | **Split** — core framework is Apache-2.0; code under `ee/` directories is Mastra Enterprise License          | Kepler Software (Mastra maintainers)                                                                                   | Yes for Apache-2.0-covered code                              | Permitted for core; restricted for `ee/`           | Permitted for core; **restricted for `ee/`** (source-available, not open source)                                           | **Yes — express patent grant for Apache-2.0-covered code**                                   | **Yes — this pass specifically checked beyond the root LICENSE and found the split** | **FACT** — Mastra's own licensing docs page directly states the `ee/` carve-out. See `research/contradictions/license-split-by-directory.md`.                               |
...
both split their license by directory/package rather than applying one
license uniformly. Neither split was found by checking the root LICENSE
file alone; both required reading further (a secondary blog post for
LangGraph, the framework's own licensing docs page for Mastra). This
promotes "check the whole tree, not just the root LICENSE" from a one-off
